"""
Generate consensus benchmark report for daisy_mamil3 instance 7 (noise 1e-4).
Outputs a Markdown report comparing best-fit baseline, family consensus, and family consensus + refit.
"""
...
println("Running consensus benchmark...")
artifact = with_logger(NullLogger()) do
    ODEPE._build_consensus_benchmark_artifact(
        pep;
        est_opts = est_opts,
...
report_path = joinpath(@__DIR__, "..", "artifacts", "diagnostics", "daisy_mamil3", "consensus_benchmark.md")
mkpath(dirname(report_path))
write(report_path, ODEPE._render_consensus_benchmark_markdown(artifact))
