...
include(joinpath(@__DIR__, "..", "multiplicity_complete_2026_05_19", "compute_M.jl"))
...
failures = String[]
println("system,algebraic_multiplicity")
for (name, ode) in M1_CASES
    M = branch_multiplicity(ode)
    println(name, ",", M)
    if M != 1
        push!(failures, string(name, " returned M=", M))
    end
end

if !isempty(failures)
    error("M=1 audit failed: " * join(failures, "; "))
end
