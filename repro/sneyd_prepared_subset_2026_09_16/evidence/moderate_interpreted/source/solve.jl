...
length(ARGS) in (5,6) || error("Expected SYSTEM_JSON OUTPUT_DIRECTORY SELECTION_JSON CASE SECONDS [all|none]")
input_path,out,selection_path,label,seconds_arg=ARGS[1:5]
compile_mode=length(ARGS)==6 ? ARGS[6] : "all"
compile_mode in ("all","none") || error("Unsupported compile mode")
compile_option=compile_mode=="all" ? :all : false
...
record=Dict{String,Any}("status"=>"building_system","profile_ready"=>true,
    ...
    "parameter_elimination"=>false,"compile"=>compile_mode,"coefficient_arithmetic"=>"Float64",
...
try
    ...
    setup=@timed HC.solver_startsolutions(system;start_system=:polyhedral,
        compile=compile_option,seed=UInt32(20260916),show_progress=true)
