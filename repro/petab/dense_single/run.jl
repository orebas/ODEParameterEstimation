...
record["versions"] = Dict(d.name => string(d.version) for d in values(Pkg.dependencies())
    if d.name in ("ODEParameterEstimation", "PEtab", "Symbolics", "SymbolicUtils", "ModelingToolkit",
        "HomotopyContinuation", "SIAN", "StructuralIdentifiability", "GaussianProcesses"))
record["thread_settings"] = Dict(k => get(ENV,k,"unset") for k in
    ("JULIA_NUM_THREADS", "OMP_NUM_THREADS", "OPENBLAS_NUM_THREADS", "MKL_NUM_THREADS"))
checkpoint() = (record["elapsed_seconds"] = (time_ns()-started_ns)/1e9; write_json(joinpath(out,"result.json"), record))
checkpoint()
...
record["status"] in ("failed", "interrupted") && exit(1)
