...
def main():
    ...
    si_probe = args.model in ("si_sneyd", "si_fujita")
    if si_probe != (args.si_mode is not None):
        parser.error("--si-mode is required only for SI probes")
    ...
    if si_probe:
        source = out / "source"
        source.mkdir()
        for filename in ("probe_si_cost.jl", "common.jl", "supervise.py"):
            shutil.copyfile(here / filename, source / filename)
        if args.model == "si_sneyd":
            name = "Sneyd_PNAS2002"
            retained = here / "evidence/followup_20260914/sneyd"
            report = retained / "odepe-sneyd-dense-20260914.json"
            data = retained / "odepe-sneyd-dense-20260914.data.json"
        else:
            name = "Fujita_SciSignal2010"
            report = here / "evidence/fujita_201_fixed.json"
            data = here / "evidence/fujita_201.data.json"
        command += [str(source / "probe_si_cost.jl"), name, args.si_mode,
                    str(report), str(data), str(out)]
    elif tracking:
        source = out / "source"
        source.mkdir()
