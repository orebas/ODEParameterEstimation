...
length(ARGS) == 2 || error("Expected START_PAIR_JSON OUTPUT_DIRECTORY")
...
try
    ...
    for mode in (:parameter, :gamma_straight), target in input["cases"][2:end]
        ...
        measurement = @timed if mode == :parameter
            HC.solve(system, [x0]; start_parameters=p0, target_parameters=p1, show_progress=false)
        else
            # Same fixed-system gamma homotopy as ODEPE._track_gamma_straight,
            # here with one seed and no fresh-solve fallback.
            HC.solve(system, system, [x0]; start_parameters=p0, target_parameters=p1,
