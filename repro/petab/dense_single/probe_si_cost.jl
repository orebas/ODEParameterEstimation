...
mode in ("local", "local_fixed", "global", "functions_absent", "functions_standard") || error("Unknown SI probe mode")
...
try
    ...
    if mode in ("local", "local_fixed")
        ...
        for seed in 20260914:20260916
            ...
            checkpoint()
            if mode == "local_fixed" && seed == 20260914
                # A coordinate-slice diagnostic only. This does not change ODEPE's
                # representative selection, or assert global/positive feasibility.
                @assert all(p in si_ode.parameters for p in basis)
                record["fixed_trials"] = Any[]
                for slice in ("ones", "distinct_rationals")
                    values = Dict(p => (slice == "ones" ? ODEPE.Nemo.QQ(1) : ODEPE.Nemo.QQ(i + 10, 10))
                        for (i,p) in enumerate(basis))
                    fixed_ode = SI.set_parameter_values(si_ode, values)
                    remaining = vcat(fixed_ode.parameters, fixed_ode.x_vars)
                    for fixed_seed in 20260914:20260916
                        Random.seed!(fixed_seed)
                        remaining_basis = ODEPE.Nemo.QQMPolyRingElem[]
                        fixed_measurement = @timed SI.assess_local_identifiability(fixed_ode;
                            funcs_to_check=remaining, prob_threshold=0.999, type=:SE,
                            trbasis=remaining_basis, loglevel=Logging.Info)
                        push!(record["fixed_trials"], Dict("slice"=>slice, "seed"=>fixed_seed,
                            "fixed_parameters"=>values, "seconds"=>fixed_measurement.time,
                            "compile_seconds"=>fixed_measurement.compile_time,
                            "local_identifiability"=>fixed_measurement.value,
                            "transcendence_basis"=>string.(remaining_basis)))
                        checkpoint()
                    end
                end
            end
