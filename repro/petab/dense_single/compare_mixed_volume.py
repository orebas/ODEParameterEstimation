...
def main():
    ...
    parser.add_argument('--ordering',choices=['original','few_terms'],default='original')
    args=parser.parse_args()
    if not 1<=len(args.sizes)<=3: parser.error('Use one to three systems')
    args.output.mkdir(parents=True,exist_ok=False)
    source=args.output/'source';source.mkdir()
    worker_source=Path(__file__).with_name('measure_polynomial_system.jl').resolve()
    shutil.copyfile(worker_source,source/worker_source.name)
    ...
    try:
        for n in args.sizes:
            ...
            command=['julia','--startup-file=no','--compiled-modules=existing',
                     str((source/worker_source.name).resolve()),
                     str((args.systems/f'system_{n:02d}.json').resolve()),str(output.resolve()),args.ordering]
