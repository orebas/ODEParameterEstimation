"""Bound a native single-experiment, single-point coefficient-estimation run."""
import argparse
import json
import os
...
def main():
    ...
    parser.add_argument("--scenario", choices=("original", "moderate"), default="original")
    ...
    for filename in ("model.jl", "run.jl", "capture_and_solve.jl", "prepare.py", "supervise.py"):
    ...
    command = ["julia", "--startup-file=no", "--compiled-modules=existing",
               str(source / "run.jl"), str(out / "spec.json"), str(out), args.scenario]
    env = os.environ.copy()
    ...
    report = {"command": command, "estimation_seconds_cap": args.seconds,
              "loading_and_preparation_seconds_cap": 600, "profile_signals": []}
    print("START", args.scenario, flush=True)
    ...
    print("END", args.scenario, report["status"], flush=True)
