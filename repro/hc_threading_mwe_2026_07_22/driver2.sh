...
# if this script dies.
#
# Usage: driver2.sh <label> <threads> <mode> <hcthr> <nworkers> <rounds> <nsolves> <stall_secs> <shape>
set -uo pipefail
cd "$(dirname "$0")"
...
for r in $(seq 1 "$ROUNDS"); do
  ...
  : > "$log"
  timeout -k 30 "$HARDCAP" julia --startup-file=no -t "$THREADS" attempt.jl \
  ...
  while kill -0 "$pid" 2>/dev/null; do
    ...
    if [ "$age" -gt "$STALL" ] && kill -0 "$pid" 2>/dev/null; then
      ...
      pending=$(awk '$1=="ATTEMPT"{if($4=="START"||$4=="SOLVE")s[$2]=1; if($4=="DONE"||$4=="ERROR")delete s[$2]} END{for(k in s)printf "%s ",k}' "$log")
  ...
  wait "$pid" 2>/dev/null
  rc=$?
  done_ct=$(grep -c ' DONE ' "$log" 2>/dev/null); done_ct=${done_ct:-0}
