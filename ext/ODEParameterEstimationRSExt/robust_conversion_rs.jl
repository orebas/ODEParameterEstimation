# RS and RationalUnivariateRepresentation dependent functions from robust_conversion.jl
...
function solve_with_rs_new(poly_system, varlist;
	...
	try
		if debug
			println("\nDEBUG [solve_with_rs_new]: Starting solve")
		...
		if debug
			println("  Cleared denominators successfully")
		end

		# Use the robust conversion method that handles special characters
		if debug
			println("  Converting to AbstractAlgebra polynomials...")
		end
		R, aa_system, var_map = robust_exprs_to_AA_polys(cleared_system, varlist)
		if debug
			println("  Conversion successful, created $(length(aa_system)) polynomials")
		end

		# Compute RUR and get separating element
		if debug
			println("  Computing Rational Univariate Representation...")
		end
		...
		if debug
			println("  RUR computed successfully, univariate poly degree: $(length(rur[1])-1)")
		end
		...
		try
			...
			if debug
				println("  Finding roots with PolynomialRoots...")
			end
			all_roots = PolynomialRoots.roots(coeffs)
			if debug
				println("  Found $(length(all_roots)) roots")
			end
			valid_solutions = 0
			...
			for (root_idx, root) in enumerate(all_roots)
				...
				if abs(f1_deriv) < 1e-14
					if debug
						println("    Skipping root due to small derivative")
					end
					continue
				end
				...
				if !isempty(solution)
					...
					valid_solutions += 1
					if debug
						println("    Added solution $valid_solutions from root $root_idx")
					end
				end
			end
		...
		# TODO: Re-enable once solve_with_nlopt is properly available
		...
		if debug
			println("  Returning $(length(solutions)) solution(s)")
		end
