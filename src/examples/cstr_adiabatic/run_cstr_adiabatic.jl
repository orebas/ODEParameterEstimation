"""
Run parameter estimation for the CSTR model with adiabatic temperature rise parameterization.

This script tests whether using ΔT_ad = Cin × dH_rhoCP as a combined parameter
resolves the identifiability issues seen in cstr_reparametrized.
"""
...
# Estimation options - matching the standard settings from run_examples.jl
...
function run_cstr_adiabatic_estimation(; opts = standard_opts)
    model_name = :cstr_adiabatic
            ...
            redirect_stdout(log_stream) do
                redirect_stderr(log_stream) do
                    try
                        ...
                        # Use the model's recommended time interval
                        ...
                        # Create options with correct time interval
