	...
	for model_symbol in collect(keys(model_dict))
		for noise_level in noise_levels
			for (run_num, pep) in results[noise_level][model_symbol]
				...
				sol, besterror, best_min_error, best_mean_error, best_median_error, best_max_error, best_approximation_error, best_rms_error = analyze_parameter_estimation_problem(pep, nooutput = false)
