	...
	for noise_level in noise_levels
		...
		for model_symbol in collect(keys(model_dict))
			...
			while length(valid_peps) < num_runs
				# Generate new random parameters and initial conditions within the interval
				...
				# Create new PEP with updated values
				newpep = ParameterEstimationProblem(
					original_pep.name,
					original_pep.model,
					original_pep.measured_quantities,
					original_pep.data_sample,
					fixed_time_interval,  # Use fixed interval instead of recommended
					original_pep.solver,
					random_p,             # Use random parameters
					random_ic,            # Use random initial conditions
					original_pep.unident_count,
				)
