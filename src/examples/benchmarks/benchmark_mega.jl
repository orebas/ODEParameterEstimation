...
function test_solver(name, solver_func, poly_system, varlist; max_time=30.0)
    println("\nTesting: $name")
    ...
    return (name=name, time_ms=t_ms, success=success, final_norm=final_norm)
end
...
function get_all_solvers()
    ...
    if :NLsolve in available_packages
        solvers["NLsolve_newton"] = function(poly_system, varlist, residual!, objective, x0)
            ...
            result = NLsolve.nlsolve(f!, x0, method=:newton)
            return result
        end
        ...
        solvers["NLsolve_trust_region"] = function(poly_system, varlist, residual!, objective, x0)
            ...
            result = NLsolve.nlsolve(f!, x0, method=:trust_region)
            return result
        end
        ...
        solvers["NLsolve_anderson"] = function(poly_system, varlist, residual!, objective, x0)
            ...
            result = NLsolve.nlsolve(f!, x0, method=:anderson, m=10)
            return result
        end
    end
    ...
    if :Metaheuristics in available_packages
        ...
        solvers["Metaheuristics_GA"] = function(poly_system, varlist, residual!, objective, x0)
            n = length(x0)
            bounds = Metaheuristics.boxconstraints(lb=fill(-10.0, n), ub=fill(10.0, n))
            result = Metaheuristics.optimize(objective, bounds, Metaheuristics.GA())
            return result
        end
    ...
    if :Evolutionary in available_packages
        solvers["Evolutionary_CMAES"] = function(poly_system, varlist, residual!, objective, x0)
            result = Evolutionary.optimize(objective, x0, Evolutionary.CMAES())
            return result
        end
        ...
        solvers["Evolutionary_GA"] = function(poly_system, varlist, residual!, objective, x0)
            ...
            lower = fill(-10.0, n)
            upper = fill(10.0, n)
            result = Evolutionary.optimize(objective, lower, upper, x0, Evolutionary.GA())
            return result
        end
        ...
        solvers["Evolutionary_DE"] = function(poly_system, varlist, residual!, objective, x0)
            ...
            lower = fill(-10.0, n)
            upper = fill(10.0, n)
            result = Evolutionary.optimize(objective, lower, upper, x0, Evolutionary.DE())
            return result
        end
    end
    ...
    if :IntervalRootFinding in available_packages && :IntervalArithmetic in available_packages
        solvers["IntervalRootFinding"] = function(poly_system, varlist, residual!, objective, x0)
            ...
            # Find roots
            roots = IntervalRootFinding.roots(F, X)
            
            if !isempty(roots)
                # Return the midpoint of the first root found
                return (u = IntervalArithmetic.mid.(roots[1].interval))
            else
                return nothing
            end
        end
    ...
    # 8. JuMP with Ipopt - Skip for now as it requires complex setup
    
    return solvers
end

# Main benchmark function
function run_mega_benchmark(poly_system, varlist)
    println("\n" * "="^80)
    ...
    for (name, solver_func) in solvers
        result = test_solver(name, solver_func, poly_system, varlist)
        push!(results, result)
...
function main()
    ...
    # Run mega benchmark
    results = run_mega_benchmark(poly_system, varlist)
