...
function run_paper_runner()
	...
	noise_levels = [1e-4]  #[0.0, 1e-8,1e-6, 1e-4, 1e-2]
	...
	for model_symbol in collect(keys(model_dict))
		for noise_level in noise_levels
			for (run_num, pep) in results[noise_level][model_symbol]
				...
				println("Running analysis for model $model_symbol run $run_num with LBFGS")
				@time sol, besterror, best_min_error, best_mean_error, best_median_error, best_max_error, best_approximation_error, best_rms_error =
					analyze_parameter_estimation_problem(pep, nooutput = false, polish_method = LBFGS, polish_maxiters = 20, try_more_methods = false)
				println("Running analysis for model $model_symbol run $run_num with NewtonTrustRegion")
				@time sol, besterror, best_min_error, best_mean_error, best_median_error, best_max_error, best_approximation_error, best_rms_error =
					analyze_parameter_estimation_problem(pep, nooutput = false, polish_method = NewtonTrustRegion, polish_maxiters = 20, try_more_methods = false)
				println("Running analysis for model $model_symbol run $run_num with KrylovTrustRegion")
				@time sol, besterror, best_min_error, best_mean_error, best_median_error, best_max_error, best_approximation_error, best_rms_error =
					analyze_parameter_estimation_problem(pep, nooutput = false, polish_method = Optim.KrylovTrustRegion, polish_maxiters = 20, try_more_methods = false)
				...
				#catch e
				#println("Warning: Analysis failed for model $model_symbol run $run_num: $e")
				# Add row with missing values for failed runs
				push!(results_df, [noise_level, string(model_symbol), run_num, missing, missing, missing, missing, missing, missing])
				#end
			end
		end
